...
func parseRemnawaveHeaders(rawHeaders, caddyAuthToken, egamesCookie string) map[string]string {
	...
	if cookie, ok := normalizeEGamesCookie(egamesCookie); ok {
		headers["Cookie"] = cookie
	} else if strings.TrimSpace(egamesCookie) != "" {
		slog.Warn("Ignoring invalid EGAMES_COOKIE; expected a single NAME=VALUE pair")
	}
	return headers
}

// normalizeEGamesCookie accepts the single cookie pair generated by the eGames
// Nginx proxy. It deliberately rejects cookie attributes and line breaks so an
// environment value cannot inject another request header.
func normalizeEGamesCookie(raw string) (string, bool) {
