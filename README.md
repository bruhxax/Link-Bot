...
Скрипт сам определяет, обслуживает ли домен встроенный или общий Caddy. При первом запуске встроенного Caddy используйте `bash ./update.sh --standalone` вместо последней команды.
После обновления проверьте адрес кабинета: `curl -I https://my.bot.example.com/mini-app/` (замените домен на свой).
